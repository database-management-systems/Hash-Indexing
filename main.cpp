...
public:
    ...
    bool lookup_record_block(long int id, Record* rc){
        Block* blk = this;
        while (blk) {
            for (int i = 0; i < blk->records.size(); i++) {
                if (blk->records[i].id == id) {
                    *rc = blk->records[i];
                    return true;
                }
            }
...
public:
    ...
    void lookup_block(int id){
        Record rc;
        unsigned long int hskey = Hashfunc(id);
        if (hskey >= big_bk.size())
            hskey -= (1 << (bits_num - 1));

        if (big_bk[hskey]->lookup_record_block(id, &rc)) {
            cout << "ID: " << rc.id << endl;
            cout<< "Name: " << rc.name << endl;
            cout<< "Bio: " << rc.bio << endl;
            cout<< "Manager ID: " <<rc.m_id << endl;
        }
        else
            cout << "We can't find the record." << endl;
    }
    int get_record_num(){
        return rec_numbers;
    }
...
public:
    ...
    void savefile(HashTable* table){
        fout.open("EmployeeIndex", ios::out);
        vector<Record> rc;
        table->dump_table(rc);
...
void search_table(HashTable *table){
    ...
    cout << "Please enter the ID you want search : ";
    cin >> id;
    cin.ignore();
    if (id >= 0){
        table->lookup_block(id);
    }
    else
        cout << "Can't find ID:   "<< id << endl;
}
...
int main(int argc, char *argv[]) {
    ...
    do{
        ...
        cout << "Enter \"L\" to check a record." << endl;
